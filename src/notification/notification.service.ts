...
export class NotificationService {
    ...
    signatories.forEach(sig => {
      ...
      if (userId) {
        this.notifyUser(userId, 'documentReadyForSigning', {
          ...
          documentTitle: sig.document?.title || sig.document?.fileName,
          message: `O documento "${sig.document?.title || sig.document?.fileName}" está pronto para sua assinatura.`,
        });
      }
    });
  }
  ...
  notifyDocumentCompleted(document: Document): void {
    ...
    const notificationData = {
      ...
      message: `O documento "${document.title || document.fileName}" foi assinado por todos.`,
    };
  ...
  notifyDocumentRejected(document: Document, rejectedBy: User, reason?: string | null): void {
    ...
    const notificationData = {
      ...
      reason: reason,
      message: `O documento "${document.title || document.fileName}" foi rejeitado por ${rejectedBy.name}. Motivo: ${reason || 'Não especificado'}`,
    };
  ...
  notifyDocumentCancelled(document: Document): void {
    ...
    const notificationData = {
      documentId: document.id,
      documentTitle: document.title || document.fileName,
      message: `O fluxo de assinatura do documento "${document.title || document.fileName}" foi cancelado.`,
    };
